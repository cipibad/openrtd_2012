...
/* Realtek Mars supports nand chip types */
...
#define RBA_PERCENT	5
#define	BB_INIT	0xF0000000
#define	RB_INIT	0xF1000000
#define	BBT_TAG	0xBB

typedef struct  __attribute__ ((__packed__)){
    unsigned int bad_block;
    unsigned int remap_block;
    unsigned char bad_block_type;
}BB_t;


typedef struct{
    unsigned char  *name;
    unsigned int id;	//id = maker_code << 8 | device_code;
 ...
 */
extern int rtk_nand_scan (struct mtd_info *mtd);
...
struct nand_chip{
	...
	//int (*read_ecc_page) (struct mtd_info *mtd, int page, int pc, u_char *data);
	//int (*read_ecc_page) (struct mtd_info *mtd, unsigned int page, u_char *data);
	int (*read_ecc_page) (struct mtd_info *mtd, unsigned int page, u_char *data, u_char *oob_buf);
	//int (*read_oob) (struct mtd_info *mtd, int page, int col, int len, u_char *buf);
	int (*read_oob) (struct mtd_info *mtd, int page, int len, u_char *buf);
	//int (*write_ecc_page) (struct mtd_info *mtd, int page, int pc, const u_char *data,
		//								const u_char *oob_buf);
	int (*write_ecc_page) (struct mtd_info *mtd, unsigned int page, const u_char *data,
										const u_char *oob_buf);										
	//int (*write_oob) (struct mtd_info *mtd, int page, int col, int len, const u_char *buf);
	int (*write_oob) (struct mtd_info *mtd, int page, int len, const u_char *buf);
	int (*erase_block) (struct mtd_info *mtd, int page);
